...
class CharacterMotorMovementGIPROXY {
...
var movement : CharacterMotorMovementGIPROXY = CharacterMotorMovementGIPROXY();

enum MovementTransferOnJumpGIPROXY {
...
class CharacterMotorJumpingGIPROXY {
...
var jumping : CharacterMotorJumpingGIPROXY = CharacterMotorJumpingGIPROXY();

class CharacterMotorMovingPlatformGIPROXY {
	var enabled : boolean = true;
	
	var movementTransfer : MovementTransferOnJumpGIPROXY = MovementTransferOnJumpGIPROXY.PermaTransfer;
...
var movingPlatform : CharacterMotorMovingPlatformGIPROXY = CharacterMotorMovingPlatformGIPROXY();

class CharacterMotorSlidingGIPROXY {
...
var sliding : CharacterMotorSlidingGIPROXY = CharacterMotorSlidingGIPROXY();
...
private function UpdateFunction () {
	...
	if (grounded && !IsGroundedTest()) {
		...
		if (movingPlatform.enabled &&
			(movingPlatform.movementTransfer == MovementTransferOnJumpGIPROXY.InitTransfer ||
			movingPlatform.movementTransfer == MovementTransferOnJumpGIPROXY.PermaTransfer)
		) {
...
private function ApplyInputVelocityChange (velocity : Vector3) {	
	...
	if (movingPlatform.enabled && movingPlatform.movementTransfer == MovementTransferOnJumpGIPROXY.PermaTransfer) {
...
private function MoveWithPlatform () : boolean {
	return (
		...
		&& (grounded || movingPlatform.movementTransfer == MovementTransferOnJumpGIPROXY.PermaLocked)
